...
FT_Face ttfloader::load_ttf_from_file(const char *name, size_t font_height) {
    // todo: better path detection
    std::vector<int8_t> readBytes = file_utilsread_bytes(std::string("fonts/") + name);
    return load_ttf_from_ptr(readBytes.data(), readBytes.size(), font_height);
}
...
FT_Face ttfloader::load_ttf_from_ptr(const void *ptr, size_t num_bytes, size_t font_height) {
